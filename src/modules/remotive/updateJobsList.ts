...
async function categorizeJobsByLocation (jobsList: Job[]): Promise<void> {
  ...
  for (const location of locationListLowered) {
    ...
    await paginateJobsList(paginateArguments)
    console.info(`[sac] ${location} sub-category updated`)
  }
  ...
  console.info('[sac] required location category completed')
}

async function saveMetadata (remotiveApi: string): Promise<void> {
  const lastUpdate = new Date().getTime()
  const metadateObject = { last_update: lastUpdate, remotive_api: remotiveApi }

  await writeJSON(`${API_PUBLIC_PATH}/remotive/index.json`, JSON.stringify(metadateObject))
  console.info('[sac] metadata saved')
}
