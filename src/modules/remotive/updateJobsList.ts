...
async function categorizeJobsListBy(category: CategorizableJobAtrrs, jobsList: Job[]) {
...
async function categorizeJobsByLocation(jobsList: Job[]) {
  ...
  for (const location of locationListLowered) {
    ...
    const otherLocationsList = jobsList.filter((job) => {
      ...
      return locationListLowered[0] !== candidateRequiredLocation &&
        locationListLowered[1] !== candidateRequiredLocation
    })
...
async function saveMetadata(remotive_api: string) {
